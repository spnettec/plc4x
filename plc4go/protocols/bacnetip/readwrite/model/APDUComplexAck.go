...
_val, _err := BACnetServiceAckParseWithBuffer(ctx, readBuffer , uint32(apduLength) - uint32(apduHeaderReduction) )
	...
	numberOfBytessegment := int(utils.InlineIf(segmentedMessage, func() any {return uint16((utils.InlineIf((bool((apduLength) > ((0)))), func() any {return uint16((uint16(apduLength) - uint16(segmentReduction)))}, func() any {return uint16(uint16(0))}).(uint16)))}, func() any {return uint16(uint16(0))}).(uint16))
