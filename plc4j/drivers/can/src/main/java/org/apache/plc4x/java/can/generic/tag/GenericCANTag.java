...
public class GenericCANTag implements PlcTag {
    ...
    public List<ArrayInfo> getArrayInfo() {
        if (arraySize > 1) {
            return Collections.singletonList(new DefaultArrayInfo(0, arraySize - 1));
        }
    ...
    static GenericCANTag create(Matcher tagQuery) {
        ...
        int arraySize = tagQuery.group("arraySize") != null ? Integer.parseInt(tagQuery.group("arraySize")) : 1;
        return new GenericCANTag(nodeId, dataType, arraySize);
    }

    @Override
    public String toString() {
        return "GenericCANTag(" + nodeId + ":" + dataType.name() + (arraySize == 1 ? "" : "[" + arraySize + "]");
    }
