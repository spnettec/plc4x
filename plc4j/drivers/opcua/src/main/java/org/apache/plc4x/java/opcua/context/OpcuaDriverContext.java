...
public class OpcuaDriverContext {
    ...
    public void initialize(String transportCode, String host, String port, String transportEndpoint,
        ...
        // Use endpoint-host / endpoint-port overrides for the OPC UA endpoint URL
        // sent in the HELLO message.  This is required when the client connects
        // through Docker port mapping or a proxy where the actual transport address
        // differs from what the OPC UA server advertises.  Milo 1.1.x validates
        // the endpoint URL in the HELLO against its registered endpoints.
        String effectiveHost = configuration.getEndpointHost() != null
            ? configuration.getEndpointHost() : host;
        String effectivePort = configuration.getEndpointPort() != null
            ? configuration.getEndpointPort().toString() : port;
        String portAddition = effectivePort != null ? ":" + effectivePort : "";
        this.endpoint = "opc." + transportCode + "://" + effectiveHost + portAddition + this.transportEndpoint;

        LOGGER.info("OPC UA endpoint URL for HELLO: {}", this.endpoint);
