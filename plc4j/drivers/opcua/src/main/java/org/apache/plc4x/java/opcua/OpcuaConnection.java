...
public class OpcuaConnection extends ConnectionBase<OpcuaConfiguration> implements OpcuaWire {
    ...
    private volatile CompletableFuture<Void> remoteDisconnect = new CompletableFuture<>();
    // Transport endpoint path extracted from the connection URL (e.g. "/milo").
    // Set by OpcuaPlcDriver before connect() is called.
    private String transportEndpoint = "";
    // PlcAuthentication passed via getConnection(url, auth) — takes priority
    // over username/password URL params (pre-merge: ConversationContext.getAuthentication()).
    private PlcAuthentication plcAuthentication;
    ...
    /**
     * Set the transport endpoint path (e.g. {@code "/milo"}) extracted from the
     * connection URL.  Called by {@link OpcuaPlcDriver} right after construction,
     * before {@link #connect()} is invoked.
     */
    void setTransportEndpoint(String transportEndpoint) {
        this.transportEndpoint = transportEndpoint != null ? transportEndpoint : "";
    }

    /**
     * Set the {@link PlcAuthentication} passed via
     * {@link org.apache.plc4x.java.api.PlcDriver#getConnection(String, PlcAuthentication)}.
     * Takes priority over username/password URL params.
     */
    void setPlcAuthentication(PlcAuthentication authentication) {
        this.plcAuthentication = authentication;
    }
    ...
    protected PlcValueHandler getValueHandler() {
        return new OpcuaPlcValueHandler();
    }
    ...
    protected void onConnect() throws PlcConnectionException {
        ...
        try {
            // Discovery fetches the server certificate — only needed for
            // encrypted modes.  Pre-merge (old SPI), discovery and connect ran
            // on SEPARATE TCP connections: the framework closed the first
            // connection after onDiscover(), then opened a fresh one for
            // onConnect().  We replicate that by creating a temporary transport
            // for discovery, so the main transport stays fresh for onConnect().
            if (configuration.isDiscovery()
                ...
                LOGGER.debug("Discovering endpoints on a temporary connection");
                discoverOnTemporaryTransport(remote, tcp.getConfiguration());
                // Discovery set the server certificate on the configuration.
                // Re-open the keystore so the driverContext picks up the
                // discovered certificate and computes its thumbprint.
                driverContext.openKeyStore(configuration);
            }

            // Set up the main transport — no HELLO has been sent on it yet.
            remoteDisconnect = new CompletableFuture<>();
    ...
    private PlcAuthentication getAuthentication() {
        // PlcAuthentication passed via getConnection(url, auth) takes priority
        // over username/password URL params — matches pre-merge behavior where
        // ConversationContext.getAuthentication() overrode URL params.
    ...
    /**
     * Runs OPC UA endpoint discovery on a <em>temporary</em> TCP connection,
     * replicating the pre-merge (old SPI) behavior where the framework executed
     * {@code onDiscover()} on one TCP connection, closed it, then opened a
     * fresh connection for {@code onConnect()}.
     *
     * <p>This is required because Milo 1.1.x validates the HELLO endpoint URL
     * and, more importantly, the server's Netty pipeline advances past the
     * HELLO handler after the first HELLO.  A second HELLO on the same TCP
     * connection (as {@code onConnect()} would send) is rejected with
     * {@code Bad_TcpMessageTypeInvalid}.</p>
     */
    private void discoverOnTemporaryTransport(InetSocketAddress remote,
                                               TcpTransportConfiguration transportConfig)
            throws Exception {
        TcpTransportInstance discoveryTransport = new TcpTransportInstance(
            remote, transportConfig, auditLog);
        try {
            OpcuaMessageCodec discoveryCodec = new OpcuaMessageCodec(
                discoveryTransport, this::handleIncoming);

            // Temporarily swap the codec so sendToWire() uses the discovery transport
            OpcuaMessageCodec savedCodec = this.messageCodec;
            this.messageCodec = discoveryCodec;

            // Register async data listener on the discovery transport
            discoveryTransport.registerDataListener(() -> {
                try {
                    discoveryCodec.processIncomingData();
                } catch (MessageCodecException e) {
                    LOGGER.error("Error processing discovery data", e);
                }
            });

            Conversation discoveryConversation = new Conversation(this, driverContext, configuration);
            SecureChannel discoveryChannel = new SecureChannel(
                discoveryConversation, driverContext, configuration, getAuthentication());

            EndpointDescription endpoint = discoveryChannel.onDiscover()
            ...
            LOGGER.info("Discovery complete, server certificate obtained");

            // Restore original codec (will be replaced by main transport setup)
            this.messageCodec = savedCodec;
            listeners.clear();
        } finally {
            try {
                discoveryTransport.removeDataListener();
    ...
    public void close() throws Exception {
        ...
        if (!subscriptions.isEmpty()) {
            List<CompletableFuture<Void>> pendingSubscriptionDeletes = new ArrayList<>();
            for (OpcuaSubscriptionHandle subscriber : new ArrayList<>(subscriptions.values())) {
                pendingSubscriptionDeletes.add(subscriber.stopSubscriber()
                    .exceptionally(error -> {
                        LOGGER.warn("Error deleting OPC UA subscription during disconnect", error);
                        return null;
                    }));
            }
            try {
                CompletableFuture.allOf(pendingSubscriptionDeletes.toArray(new CompletableFuture<?>[0]))
                    .get(configuration.getNegotiationTimeout(), TimeUnit.MILLISECONDS);
            } catch (TimeoutException e) {
                LOGGER.warn("Timed out waiting for OPC UA subscriptions to stop during disconnect", e);
            } catch (Exception e) {
                LOGGER.warn("Error waiting for OPC UA subscriptions to stop during disconnect", e);
            }
        }
        if (channel != null) {
            try {
                channel.onDisconnect().get(configuration.getNegotiationTimeout(), TimeUnit.MILLISECONDS);
            } catch (TimeoutException e) {
                LOGGER.warn("Timed out waiting for OPC UA secure channel disconnect", e);
            } catch (Exception e) {
                LOGGER.warn("Error during secure channel disconnect", e);
            }
        }
        CompletableFuture<Void> disconnectFuture = remoteDisconnect;
        if (disconnectFuture != null && !disconnectFuture.isDone()) {
            try {
                disconnectFuture.get(Math.min(configuration.getNegotiationTimeout(), 1000L), TimeUnit.MILLISECONDS);
            } catch (TimeoutException e) {
                LOGGER.debug("Timed out waiting for remote OPC UA disconnect after CloseSecureChannel");
            } catch (Exception e) {
                LOGGER.debug("Error waiting for remote OPC UA disconnect after CloseSecureChannel", e);
            }
        }
        stopReceiving();
    ...
    public void fireDisconnected() {
        connected = false;
        remoteDisconnect.complete(null);
    }
    ...
    protected void onTransportDisconnected(Throwable cause) {
        ...
        connected = false;
        if (cause != null) {
            remoteDisconnect.completeExceptionally(cause);
        } else {
            remoteDisconnect.complete(null);
        }
    }
        ...
        switch (dataType) {
            ...
            case TIME:
                ...
                return new VariantInt32(length != 1,
                    ...
                    tmpTime);
            default:
                // Tag type could not be inferred (e.g. PlcNull for a
                // non-existent node).  Send a null Variant so the server
                // can respond with the proper status (BadNodeIdUnknown etc.)
                // instead of crashing the entire write request.
                LOGGER.warn("Cannot infer write data type for tag type {}, sending null variant", dataType);
                return new VariantNull(false, false, null, java.util.Collections.emptyList());
        }
    }
    ...
    protected CompletableFuture<PlcWriteResponse> onWrite(PlcWriteRequest writeRequest) {
        ...
        // Track tags that had parse/value errors (DefaultPlcTagErrorItem stores
        // null tag + an error response code). We skip them in the OPC UA request
        // and inject their pre-known error code into the response.
        Map<String, PlcResponseCode> preErrors = new LinkedHashMap<>();
        List<String> sentTagNames = new ArrayList<>();
        for (String tagName : request.getTagNames()) {
            OpcuaTag tag = (OpcuaTag) request.getTag(tagName);
            if (tag == null) {
                // Tag parse or value conversion failed during request build
                PlcResponseCode errorCode = request.getTagResponseCode(tagName);
                preErrors.put(tagName, errorCode != null ? errorCode : PlcResponseCode.INTERNAL_ERROR);
                continue;
            }
        ...
        return conversation.submit(opcuaWriteRequest, WriteResponse.class)
            .thenApply(response -> writeResponse(request, response, sentTagNames, preErrors));
    }

    private PlcWriteResponse writeResponse(DefaultPlcWriteRequest request, WriteResponse writeResponse,
                                           List<String> sentTagNames, Map<String, PlcResponseCode> preErrors) {
        Map<String, PlcResponseCode> responseMap = new HashMap<>();
        // Inject pre-known errors for tags that failed during request build
        responseMap.putAll(preErrors);
        // Map server results to the tags that were actually sent
        List<StatusCode> results = writeResponse.getResults();
        for (int i = 0; i < sentTagNames.size(); i++) {
            String tagName = sentTagNames.get(i);
    ...
    protected CompletableFuture<PlcUnsubscriptionResponse> onUnsubscribe(PlcUnsubscriptionRequest unsubscriptionRequest) {
        List<CompletableFuture<Void>> pendingUnsubscribes = new ArrayList<>();
        unsubscriptionRequest.getSubscriptionHandles().forEach(o ->
            pendingUnsubscribes.add(((OpcuaSubscriptionHandle) o).stopSubscriber()));
        return CompletableFuture.allOf(pendingUnsubscribes.toArray(new CompletableFuture<?>[0]))
            .thenApply(ignored -> new DefaultPlcUnsubscriptionResponse(unsubscriptionRequest));
    }
