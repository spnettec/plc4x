...
class ValueCodecTest {
    ...
    private static ResolvedAdsTag tag(String dataTypeName, long size, PlcValueType pvt, int stringLen, String stringEncoding) {
        return new ResolvedAdsTag(0, 0, size, dataTypeName, pvt, stringLen, stringEncoding, Collections.emptyList());
    }
    ...
    @Test
    void scalarString_explicitEncodingRoundtrip() throws Exception {
        ResolvedAdsTag t = tag("STRING(4)", 5, PlcValueType.STRING, 4, "GBK");
        PlcValue v = new PlcSTRING("中文");
        byte[] bytes = encode(t, v);
        assertArrayEquals("中文".getBytes(Charset.forName("GBK")), Arrays.copyOf(bytes, 4));
