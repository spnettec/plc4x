    ...
    public CompletableFuture<PlcBrowseResponse> browseWithInterceptor(PlcBrowseRequest browseRequest, PlcBrowseRequestInterceptor interceptor) {
        ...
        for (String queryName : browseRequest.getQueryNames()) {
            ...
            for (AdsSymbolTableEntry symbol : symbolTable.values()) {
                ...
                DefaultPlcBrowseItem item = new DefaultPlcBrowseItem(new SymbolicAdsTag(symbol.getName(), plcValueType, arrayInfo, "AUTO"), symbol.getName(),
    ...
    protected List<PlcBrowseItem> getBrowseItems(String basePath, long baseGroupId, long baseOffset, boolean parentWritable, AdsDataTypeTableEntry dataType) {
        ...
        for (AdsDataTypeTableEntry child : dataType.getChildren()) {
            ...
            values.add(new DefaultPlcBrowseItem(new SymbolicAdsTag(
                basePath + "." + child.getMainName(), plc4xPlcValueType, arrayInfo, "AUTO"), child.getMainName(),
    ...
    private PlcResponseItem<PlcValue> parseResponseItem(DirectAdsTag tag, ReadBuffer readBuffer) {
        try {
            ...
            if (tag.getNumberOfElements() == 1) {
                ...
                return new DefaultPlcResponseItem<>(PlcResponseCode.OK, parsePlcValue(plcValueType, adsDataTypeTableEntry, singleStringLength, readBuffer, tag.getStringEncoding()));
            } else {
                // Fetch all
...
/*                        DirectAdsTag directAdsTag = new DirectAdsTag(
    ...
    protected DirectAdsTag getDirectAdsTagForSymbolicName(PlcTag tag) {
        ...
        if (addressParts.length < 2) {
            ...
            return new DirectAdsTag(adsSymbolTableEntry.getGroup(), adsSymbolTableEntry.getOffset(),
                dataTypeTableEntry.getMainName(), dataTypeTableEntry.getArrayDimensions(), symbolicAdsTag.getStringEncoding(adsSymbolTableEntry.getDataTypeName()));
        }
    ...
    protected DirectAdsTag resolveDirectAdsTagForSymbolicNameFromDataType(List<String> remainingAddressParts, long currentGroup, long currentOffset, AdsDataTypeTableEntry adsDataTypeTableEntry,String stringEncoding) {
        if (remainingAddressParts.isEmpty()) {
            ...
            if (adsDataTypeTableEntry.getDataType() == AdsDataType.CHAR.getValue()) {
                ...
                return new DirectAdsStringTag(currentGroup, currentOffset, adsDataTypeTableEntry.getMainName(), stringLength, 1, stringEncoding);
                ...
                return new DirectAdsTag(currentGroup, currentOffset, adsDataTypeTableEntry.getMainName(), 1, stringEncoding);
            }
        }
        ...
        for (AdsDataTypeTableEntry child : adsDataTypeTableEntry.getChildren()) {
            if (child.getMainName().equals(remainingAddressParts.get(0))) {
                ...
                return resolveDirectAdsTagForSymbolicNameFromDataType(
                    ...
                    currentGroup, currentOffset + child.getOffset(), childAdsDataTypeTableEntry,stringEncoding);
            }
        }
