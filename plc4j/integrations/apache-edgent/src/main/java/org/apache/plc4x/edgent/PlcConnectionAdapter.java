...
public class PlcConnectionAdapter implements AutoCloseable {
    ...
    <T> Supplier<T> newSupplier(Class<T> genericDatatype, PlcValueType clientDatatype, String fieldQuery) {
    ...
    <T> Supplier<List<T>> newListSupplier(Class<T> genericDatatype, PlcValueType clientDatatype, String fieldQuery) {
        // satisfy sonar's "Reduce number of anonymous class lines" code smell
    ...
    private class MySupplier<T> implements Supplier<T> {
        ...
        private PlcValueType clientDatatype;
        private String fieldQuery;

        MySupplier(Class<T> genericDatatype, PlcValueType clientDatatype, String fieldQuery) {
    ...
    private class MyListSupplier<T> implements Supplier<List<T>> {
        ...
        MyListSupplier(Class<T> genericDatatype, PlcValueType clientDatatype, String fieldQuery) {
            this.genericDatatype = genericDatatype;
            this.clientDatatype = clientDatatype;
            this.fieldQuery = fieldQuery;
        }
    ...
    <T> Consumer<T> newJsonConsumer(Class<T> genericDatatype, PlcValueType clientDatatype, String fieldQuery) {
        return new ObjectConsumer<>(genericDatatype, clientDatatype, fieldQuery);
    }

    <T> Consumer<JsonObject> newJsonConsumer(PlcValueType clientDatatype, Function<JsonObject, String> fieldQueryFn, Function<JsonObject, T> valueFn) {
        return new JsonConsumer<>(clientDatatype, fieldQueryFn, valueFn);
    }

    private abstract class BaseConsumer<T> implements Consumer<T> {

        protected void write(PlcValueType clientDatatype, String fieldQuery, Object fieldValue) {
            PlcConnection connection = null;
            try {
                connection = getConnection();
        ...
        private void addItem(PlcWriteRequest.Builder builder,
                             PlcValueType clientDatatype, String fieldQuery, Object fieldValue) {
            switch (clientDatatype) {
                case BYTE:
    ...
    private class ObjectConsumer<T> extends BaseConsumer<T> {
        ...
        ObjectConsumer(Class<T> genericDatatype, PlcValueType clientDatatype, String fieldQuery) {
            this.clientDatatype = clientDatatype;
            this.fieldQuery = fieldQuery;
        }
    ...
    private class JsonConsumer<T> extends BaseConsumer<JsonObject> {
        ...
        JsonConsumer(PlcValueType clientDatatype, Function<JsonObject, String> fieldQueryFn, Function<JsonObject, T> fieldValueFn) {
            this.clientDatatype = clientDatatype;
